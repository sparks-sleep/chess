...
//从棋盘上拿走一枚棋子
func (p *Position) delPiece(sq, pc int) {
	p.ucpcSquares[sq] = 0
...
//repStatus 检测重复局面
func (p *Position) repStatus(nRecur int) int {
...
func (p *Position) generateMoves(mvs []int, bCapture bool) int {
...
//probeHash 提取置换表项
func (p *Position) probeHash(vlAlpha, vlBeta, nDepth int) (int, int) {
...
func (p *Position) initSort(mvHash int, s *SortStruct) {
	if s == nil {
		return
	}
...
//nextSort 得到下一个走法
func (p *Position) nextSort(s *SortStruct) int {
	if s == nil {
		return 0
	}
...
func (p *Position) setBestMove(mv, nDepth int) {
...
func (p *Position) searchFull(vlAlpha, vlBeta, nDepth int, bNoNull bool) int {
	vl, mvHash := 0, 0
...
//searchBook 搜索开局库
func (p *Position) searchBook() int {
	bkToSearch := &BookItem{}
	mvs := make([]int, MaxGenMoves)
...
//mirror 对局面镜像
func (p *Position) mirror(posMirror *Position) {
	pc := 0
...
func (p *Position) searchRoot(nDepth int) int {
	vl, nNewDepth := 0, 0
	vlBest := -MateValue
...
//repValue 重复局面分值
func (p *Position) repValue(nRepStatus int) int {
	vlReturn := 0
	if nRepStatus&2 != 0 {
...
//RecordHash 保存置换表项
func (p *Position) RecordHash(nFlag, vl, nDepth, mv int) {
	hsh := p.search.hashTable[p.zobr.dwKey&(HashSize-1)]
